...
export interface Usuario { id: string; nombre: string; hashContrasena: string }
export class RepositorioUsuariosMemoria {
  ...
  crear(nombre: string, contrasena: string): Usuario {
    if (!nombre.trim() || contrasena.length < 8) throw new Error('Nombre y contraseña de al menos 8 caracteres son obligatorios');
    if ([...this.usuarios.values()].some((usuario) => usuario.nombre === nombre)) throw new Error('El usuario ya existe');
    ...
    this.usuarios.set(usuario.id, usuario); return usuario;
  }
  autenticar(nombre: string, contrasena: string): Usuario | undefined {
    ...
    return esperado.length === recibido.length && timingSafeEqual(esperado, recibido) ? usuario : undefined;
  }
}
