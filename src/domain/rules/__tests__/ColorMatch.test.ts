...
describe('compatibilidad de color', () => {
  ...
  const multi = new CartaHeroe({
    id: 'h2',
    nombre: 'Capitana Marvel',
    color: null,
    esMulticolor: true,
  });
  const intangible = new CartaHeroe({
    id: 'h3',
    nombre: 'Visión',
    color: null,
    esIntangible: true,
  ...
  it('exige el mismo color salvo para héroes multicolor', () => {
    expect(
      poderCompatibleConHeroe(new CartaPoder({ id: 'p1', nombre: 'Poder', color: 'rojo' }), rojo),
    ).toBe(true);
    expect(
      poderCompatibleConHeroe(new CartaPoder({ id: 'p2', nombre: 'Poder', color: 'verde' }), rojo),
    ).toBe(false);
    expect(
      poderCompatibleConHeroe(new CartaPoder({ id: 'p3', nombre: 'Poder', color: 'verde' }), multi),
    ).toBe(true);
  });
