...
describe('API de partidas', () => {
  it('informa de salud y crea una partida con manos iniciales', async () => {
    ...
    const cuerpo = respuesta.json() as { id: string; partida: { turnoDe: string; jugadores: { mano: unknown[] }[] } };
    expect(cuerpo.id).toBeTruthy();
    expect(cuerpo.partida.turnoDe).toBe('ana');
    expect(cuerpo.partida.jugadores.every((jugador) => jugador.mano.length === 3)).toBe(true);
  });
  ...
  it('rechaza partidas y jugadas inválidas con errores HTTP', async () => {
    ...
    expect((await app.inject({ method: 'GET', url: '/partidas/inexistente' })).statusCode).toBe(404);
  });
});
