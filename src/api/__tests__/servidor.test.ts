...
afterEach(async () => {
  await Promise.all(servidores.splice(0).map((servidor) => servidor.close()));
});
...
describe('API de partidas', () => {
  it('informa de salud y crea una partida con manos iniciales', async () => {
    ...
    const cuerpo = respuesta.json() as {
      id: string;
      partida: { turnoDe: string; jugadores: { cartasEnMano: number }[] };
    };
  ...
  it('rechaza partidas y jugadas inválidas con errores HTTP', async () => {
    const app = servidor();
    expect(
      (await app.inject({ method: 'POST', url: '/partidas', payload: { jugadores: [] } }))
        .statusCode,
    ).toBe(400);
    expect((await app.inject({ method: 'GET', url: '/partidas/inexistente' })).statusCode).toBe(
      401,
    );
  });
});
