...
class SinBarajar implements IBarajador { barajar<T>(cartas: readonly T[]): T[] { return [...cartas]; } }
...
describe('JugadorMaquina', () => {
  it('descarta una carta válida y cede el turno', () => {
    ...
    const cartas = Array.from({ length: 7 }, (_, i) => new CartaPoder({ id: `p${i}`, nombre: `P${i}`, color: 'rojo' }));
    const partida = Partida.iniciar({ jugadores: [maquina, humana], mazo: new Mazo(cartas, new SinBarajar()) });
